...
```{r}
# show me
edbf_term_counts$migword <- as.integer(grepl("emigration|immigrant|incomer|migration|transmigrant", 
                                            x = edbf_term_counts$word))

head(edbf_term_counts)
```
```{r}
edbf_migcounts <- edbf_term_counts %>%
  group_by(year) %>%
  mutate(year_total = sum(n)) %>%
  filter(migword==1) %>%
  summarise(sum_mig = sum(n),
            year_total= min(year_total))
```

```{r}
ggplot(edbf_migcounts, aes(year, sum_mig / year_total, group=1)) +
  geom_line() +
  xlab("Year") +
  ylab("% migrant-related words") +
  scale_y_continuous(labels = scales::percent_format(),
                     expand = c(0, 0), limits = c(0, NA)) +
  theme_tufte(base_family = "Helvetica") 
```
