...
我们的下一个任务是缩小这个数据集的大小，只包括我们需要的那些列。但首先我们可以检查它以查看现有的列名称是什么，以及每个变量是如何编码的。为此，我们可以首先调用：
```{r}
colnames(edbfdata)
```
...
```{r}
#get year and word for every word and date pair in the dataset每个单词和日期对应的年份和单词
tidy_des <- evdes %>% 
  mutate(desc = tolower(description)) %>% #将 description 变量（文本）转换为小写，以避免大小写影响后续分析
  unnest_tokens(word, desc) %>% #拆分 `desc` 中的文本，按单词进行分割
  filter(str_detect(word, "[a-z]")) #过滤掉不包含字母的“单词”
...
去除停用词（stop words），以提高文本分析的质量
```{r}
tidy_des <- tidy_des %>%
    filter(!word %in% stop_words$word)
#stop_words 是 tidytext 包提供的英语停用词表，其中包含 "the"、"is"、"and"、"of" 等高频但信息量较低的单词。
#!word %in% stop_words$word：
# 检查 word 是否在 stop_words$word 这个列表中。
# ! 取反，表示保留不在停用词表中的单词。
```
...
我们可以看到，最常见的单词之一是“rsquo”，这是撇号的 HTML 编码。显然，我们需要进一步清理数据。这是 文本分析中的一个常见问题，如果您想进行可靠的稳健文本分析，这是一个关键步骤。我们将再次使用 filter 命令，指定我们只保留未包含在字符串 `r c("rsquo", "em", "ndash", "nbsp", "lsquo") 中的单词。
...
#grepl() 用于检查字符串是否匹配某个模式。
#pattern = "women|feminist|feminism|gender|harassment|sexism|sexist"
#这是一个正则表达式，表示只要 word 包含其中任何一个词，就返回 TRUE

#as.integer(grepl(...))
#grepl() 返回 TRUE 或 FALSE，as.integer() 将其转换为 0 或 1：
#匹配成功 → 1；匹配失败 → 0

#edbf_term_counts$womword在edbf_term_counts中创建新的一列womword
```
...
edbf_counts <- edbf_term_counts %>%
  group_by(year) %>%
  mutate(year_total = sum(n)) %>% #计算每年所有单词的总出现次数，并添加新列 year_total
  filter(womword==1) %>% #仅保留 womword == 1 的行，即只保留与性别相关的词
  summarise(sum_wom = sum(n),
            year_total= min(year_total)) 
#sum_wom = sum(n): 计算该年性别相关词的总频次。
#year_total = min(year_total):
  #year_total 是 mutate() 生成的，但 summarise() 需要选取一个值。
  #min(year_total) 确保正确提取该年的 year_total（实际上这里 max() 也可以，因其值相同）
...
edbf_term_counts$raceword <- as.integer(grepl("race|ethnicity|black|white|asian|latino|african|caucasian|hispanic|minority", 
                                            x = edbf_term_counts$word))

#race_keywords <- c("race", "ethnicity", "black", "white", "asian", "latino", "african", "caucasian", "hispanic", "minority")
```
...
```{r}
edbf_racecount <- edbf_term_counts %>%
  group_by(year) %>%
  mutate(year_total = sum(n)) %>% 
  filter(raceword==1) %>% 
...
```{r, warning=FALSE}
ggplot(edbf_racecount, aes(year, sum_race / year_total, group=factor(1))) +
  geom_line() +
  xlab("Year") +
